...
function IOSNavBar({ title = 'Title', dark = false, trailingIcon = true }) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        padding: '0 16px', fontFamily: '-apple-system, system-ui',
...
function IOSList({ header, children, dark = false }) {
  ...
  return (
    <div>
      {header && (
        <div style={{
          fontFamily: '-apple-system, system-ui', fontSize: 13, color: hc,
          textTransform: 'uppercase', padding: '8px 36px 6px', letterSpacing: -0.08,
        }}>{header}</div>
      )}
...
function IOSKeyboard({ dark = false }) {
  ...
  const key = (content, { w, flex, ret, fs = 25, k } = {}) => (
    <div key={k} style={{
      height: 42, borderRadius: 8.5, flex: flex ? 1 : undefined, width: w, minWidth: 0,
      background: ret ? '#08f' : keyBg, boxShadow: '0 1px 0 rgba(0,0,0,0.075)',
      display: 'flex', alignItems: 'center', justifyContent: 'center',
